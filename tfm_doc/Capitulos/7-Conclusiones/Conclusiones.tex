...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.62\textwidth}p{0.28\textwidth}}
        ...
        \hline
        O1: Revisión bibliográfica de SNNs y detección de anomalías & Sí \\
        O2: Diseño de arquitectura SNN optimizada/híbrida & Sí \\
        O3: Implementación sostenible (eficiencia / sparsity) & Parcial \\
        O4: Validación experimental en datasets reales (IOPS, CalIt2) & Sí \\
        O5: Comparación con modelos tradicionales / TSFEDL & Sí \\
        O6: Optimización automatizada (Optuna sobre hiperparámetros) & Sí \\ 
...
Comentarios:
\begin{itemize}
    \item \textbf{Objetivo más destacado}: O6 (Optimización automatizada) - Se logró implementar exitosamente la optimización bayesiana con Optuna TPE, ejecutando 100 trials por configuración y logrando convergencia estable de los hiperparámetros.
...
\begin{enumerate}
    ...
    \item Comparativa empírica contra el modelo base (SNN A--B) y contra baselines de \texttt{TSFEDL} mostrando que, aunque los modelos TSFEDL superan en rendimiento (F1=0.436 vs 0.060 en IOPS, F1=0.704 vs 0.418 en CalIt2), las SNN mantienen ventajas en eficiencia temporal (12-15 ms por ventana) y potencial para computación neuromórfica.
...
\begin{itemize}
    \item La capa convolucional no mejora significativamente el F1-score respecto a la SNN base en datasets altamente desbalanceados como IOPS, pero mantiene robustez en CalIt2 con F1-scores estables (0.417-0.418) independientemente del número de neuronas (100-400).
    \item El modo de procesamiento \texttt{direct} fue consistentemente seleccionado por Optuna como óptimo en CalIt2, junto con kernels \texttt{mexican\_hat} y \texttt{gaussian} con tamaños 5-7, indicando preferencia por suavizado espacial en lugar de detección de bordes.
...
\begin{itemize}
    ...
    \item \textbf{Número de trials}: 100 trials por configuración pueden ser insuficientes para explorar completamente el espacio hiperparamétrico, especialmente en el caso de IOPS donde la convergencia fue problemática debido al extremo desbalanceo.
...
\begin{itemize}
    ...
    \item \textbf{Conclusión}: Comparaciones con TSFEDL limitadas a cuatro modelos específicos (OhShuLih, KhanZulfiqar, ZhengZhenyu, WeiXiaoyan), sin acceso a mediciones de tiempo de inferencia o consumo energético de los baselines.
\end{itemize}

\section{Conclusiones finales}
En síntesis, este trabajo ha demostrado que las arquitecturas híbridas SNN-CNN, aunque no superan consistentemente a los métodos de deep learning tradicionales en términos de F1-score, ofrecen un compromiso interesante entre rendimiento y eficiencia computacional. La arquitectura propuesta logró F1-scores de 0.418 en CalIt2, manteniéndose competitiva en datasets moderadamente balanceados, mientras que en IOPS reveló las limitaciones inherentes de las SNN para manejar desbalanceos extremos.
